...
if NOT DEFINED UWPBUILD (set UWPBUILD=%~dp0)
if NOT DEFINED EXTERNALS_DIR (set EXTERNALS_DIR=%UWPBUILD%\..\externals)
...
for %%e in (%libraries%) do (
    if exist "%EXTERNALS_DIR%\%%e" (
        ...
        echo.Fetching %%e...
        %PYTHON% -E "%UWPBUILD%\get_external.py" -O %ORG% -e "%EXTERNALS_DIR%" %%e
    )
)
...
for %%b in (%binaries%) do (
    if exist "%EXTERNALS_DIR%\%%b" (
        ...
        echo.Fetching %%b...
        %PYTHON% -E "%UWPBUILD%\get_external.py" -b -O %ORG% -e "%EXTERNALS_DIR%" %%b
    )
)
...
echo.with UWPbuild\get_external.py.
echo.
